import { prettyMark } from "../src/prettyMark";

describe("prettyMark", () => {
  test("syntax highlighting", async () => {
    const input = "```javascript\nconst x = 1;\n```";
    const output = await prettyMark(input, true);
  ...
  test("GitHub-style IDs", async () => {
    ...
    const output = await prettyMark(input, false, true);
  ...
  test("handles empty input", async () => {
    const input = "";
    const output = await prettyMark(input);
    expect(output).toBe("");
  });
